...
exports.addChatImage = async (req, res) => {
    try {
        ...
        res.status(200).json({ message: "image saved to database succesfully" })
...
exports.getChats = async (req, res) => {
    try {
        ...
        let groupId = req.header('groupId');
        if (groupId == '0') { groupId = null; }
        const lastMessageId = req.query.lastChatId || 0;
        ...
        res.status(200).json({ chats, message: "User chat History Fetched" })
